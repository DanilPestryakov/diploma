function [ctrmax,argmax,envs,ccode] = ctrsolvty(infA,supA,infb,supb,varargin)
%
%   Вычисление максимума распознающего функционала управляемого множества
%   решений для интервальной системы линейных алгебраических уравнений.
%
%   CTRSOLVTY(infA, supA, infb, supb) выдаёт значение максимума распознающего
%   функционала управляемого множества решений для интервальной системы линейных
...
%   максимума - управляемое решение или псевдорешение интервальной линейной
%   системы Ax = b, имеющее наибольшую меру разрешимости, а также  заключение
%   о пустоте/непустоте управляемого множества решений и диагностику работы.
%
%   Синтаксис вызова:
...
%            ctrmax - значение максимума распознающего функционала;
%            argmax - доставляющий его вектор значений аргумента,который
%                     лежит в управляемом множестве решений при ctrmax>=0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%    Эта  программа  выполняет  исследование управляемого множества решений
...
%    управляемого множества решений этой системы. См. подробности в
%
...
%
HorLine = '-------------------------------------------------------------';
TitLine = 'Протокол максимизации распознающего функционала Ctr';
TabLine = 'Шаг        Ctr(x)         Ctr(xx)   ВычФун/шаг  ВычФун';
...
%
    ...
    sups = bc - mid_sum + br;
    rad_sum = Ar * absx;
    tt = weight .* (rad_sum - max(abs(infs), abs(sups)));
...
disp(' ');
if ctrmax >= 0
    disp(' Управляемое множество решений интервальной линейной системы непусто ')
else
    disp(' Управляеоме множество решений интервальной линейной системы пусто ')
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

disp(' ');
if ( ctrmax < 0. && abs(ctrmax/epsx) < 10 )
    ...
    disp(' задачи об управляемом множестве');
    disp(' ');
end
